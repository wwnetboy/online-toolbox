...
<script setup lang="ts">
  ...
  // 检测是否为前台路由（不以 /admin 开头）
  const isPortalRoute = computed(() => !route.path.startsWith('/admin'))
...
<style lang="scss" scoped>
  ...
  .history-btn:hover :deep(.art-svg-icon) {
    animation: rotateCCW90 0.5s;
  }

  @keyframes rotateCCW90 {
    0% {
      transform: rotate(0);
    }
