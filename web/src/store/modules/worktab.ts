...
export const useWorktabStore = defineStore(
  ...
  () => {
    ...
    const safeRouterPush = (tab: Partial<WorkTab>): void => {
      if (!tab.path) {
        if (DEBUG) console.warn('尝试跳转到无效路径的标签页')
        return
      }
    ...
    const openTab = (tab: WorkTab): void => {
      if (!tab.path) {
        if (DEBUG) console.warn('尝试打开无效的标签页')
        return
      }
    ...
    const removeTab = (path: string): void => {
      ...
      if (targetIndex === -1) {
        if (DEBUG) console.warn(`尝试关闭不存在的标签页: ${path}`)
        return
      }

      if (targetTab && !isTabClosable(targetTab)) {
        if (DEBUG) console.warn(`尝试关闭固定标签页: ${path}`)
        return
      }
    ...
    const removeLeft = (path: string): void => {
      ...
      if (targetIndex === -1) {
        if (DEBUG) console.warn(`尝试关闭左侧标签页，但目标标签页不存在: ${path}`)
        return
      }
      ...
      if (closableLeftTabs.length === 0) {
        if (DEBUG) console.warn('左侧没有可关闭的标签页')
        return
      }
    ...
    const removeRight = (path: string): void => {
      ...
      if (targetIndex === -1) {
        if (DEBUG) console.warn(`尝试关闭右侧标签页，但目标标签页不存在: ${path}`)
        return
      }
      ...
      if (closableRightTabs.length === 0) {
        if (DEBUG) console.warn('右侧没有可关闭的标签页')
        return
      }
    ...
    const removeOthers = (path: string): void => {
      ...
      if (!targetTab) {
        if (DEBUG) console.warn(`尝试关闭其他标签页，但目标标签页不存在: ${path}`)
        return
      }
      ...
      if (closableTabs.length === 0) {
        if (DEBUG) console.warn('没有其他可关闭的标签页')
        return
      }
    ...
    const removeAll = (): void => {
      ...
      if (closableTabs.length === 0) {
        if (DEBUG) console.warn('没有可关闭的标签页')
        return
      }
    ...
    const toggleFixedTab = (path: string): void => {
      ...
      if (targetIndex === -1) {
        if (DEBUG) console.warn(`尝试切换不存在标签页的固定状态: ${path}`)
        return
      }
    ...
    const validateWorktabs = (routerInstance: Router): void => {
      try {
        ...
        if (validTabs.length !== opened.value.length) {
          if (DEBUG) console.warn('发现无效的标签页路由，已自动清理')
        ...
        if (!isCurrentValid && validTabs.length > 0) {
          if (DEBUG) console.warn('当前激活标签无效，已自动切换')
    ...
    const setIsAdminRoute = (value: boolean): void => {
