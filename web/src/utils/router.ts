...
/**
 * 获取系统名称（优先使用系统设置，否则使用默认配置）
 */
export const getSystemName = (): string => {
  try {
    const siteSettingsStore = useSiteSettingsStore()
    siteSettingsStore.initSettings()
    return siteSettingsStore.settings.basic.siteName || AppConfig.systemInfo.name
  } catch {
...
export const setPageTitle = (to: RouteLocationNormalized): void => {
  ...
  if (title) {
    setTimeout(() => {
      document.title = `${formatMenuTitle(String(title))} - ${getSystemName()}`
    }, 150)
  }
}
