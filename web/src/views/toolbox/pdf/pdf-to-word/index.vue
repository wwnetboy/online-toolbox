<template>
  <ToolPageLayout
    ...
    feature-name="PDF转Word"
    :title="toolName || 'PDF转Word'"
    :description="toolDescription || '将PDF文档转换为可编辑的Word格式(.docx)'"
    :icon="toolIcon"
  ...
  >
          ...
          >
            ...
            <template #actions>
              <ElButton type="primary" @click="handleDownload">
                <ElIcon class="mr-1"><Download /></ElIcon>下载Word
              </ElButton>
    ...
    <template #introduction>
      <div class="text-sm text-g-600 leading-relaxed">
        <p class="mb-3">PDF转Word工具可以将PDF文档转换为可编辑的Word格式。</p>
...
<script setup lang="ts">
  ...
  import { createPdfToWordProcessor } from '@/processors/pdf/convert'

  defineOptions({ name: 'PdfToWordPage' })
  ...
  // 工具处理器
  const processor = createPdfToWordProcessor()
  const {
    isProcessing,
    progress,
    hasResult,
    ...
    featureId: 'pdf-to-word',
    featureName: 'PDF转Word',
    successMessage: '转换完成！',
    errorMessage: '转换失败'
  })
  ...
  const outputFileName = computed(() => {
    if (!files.value[0]?.name) return 'output.docx'
    const name = files.value[0].name.replace(/\.pdf$/i, '')
    return `${name}.docx`
  })
