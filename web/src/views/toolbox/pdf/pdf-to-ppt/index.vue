<template>
  <ToolPageLayout
    feature-id="pdf-to-ppt"
    feature-name="PDF转PPT"
    :title="toolName || 'PDF转PPT'"
    :description="toolDescription || '将PDF文档转换为PowerPoint演示文稿(.pptx)'"
    :icon="toolIcon"
  ...
  >
          ...
          >
            ...
            <template #actions>
              <ElButton type="primary" @click="handleDownload">
                <ElIcon class="mr-1"><Download /></ElIcon>下载PPT
              </ElButton>
    ...
    <template #introduction>
      <div class="text-sm text-g-600 leading-relaxed">
        <p class="mb-3">PDF转PPT工具可以将PDF文档转换为PowerPoint演示文稿。</p>
...
<script setup lang="ts">
  ...
  import { createPdfToPptProcessor } from '@/processors/pdf/convert'

  defineOptions({ name: 'PdfToPptPage' })
  ...
  // 工具处理器
  const processor = createPdfToPptProcessor()
  const {
    isProcessing,
    progress,
    hasResult,
  ...
  const outputFileName = computed(() => {
    if (!files.value[0]?.name) return 'output.pptx'
    return files.value[0].name.replace(/\.pdf$/i, '') + '.pptx'
  })
