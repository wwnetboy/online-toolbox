<template>
  <ToolPageLayout
    feature-id="pdf-from-html"
    feature-name="HTML转PDF"
    :title="toolName || 'HTML转PDF'"
    :description="toolDescription || '将HTML文件转换为PDF格式'"
    :icon="toolIcon"
    ...
    <template #content>
      ...
      >
        ...
        <template v-if="!hasFiles && !isProcessing && !hasResult">
          <Icon icon="ri:html5-fill" class="text-6xl text-[#e34f26] mb-4" />
          <p class="text-base text-g-600 mb-2">将HTML文件拖拽到此处</p>
          <p class="text-sm text-g-400 mb-4">或者</p>
          <ElButton type="primary" @click="triggerFileSelect">选择HTML文件</ElButton>
          <p class="text-xs text-g-400 mt-3">支持 .html, .htm 格式，最大10MB</p>
        </template>
        ...
        <template v-if="hasFiles && !isProcessing && !hasResult">
          <div class="file-preview">
            <div class="file-icon">
              <Icon icon="ri:html5-fill" class="text-5xl text-[#e34f26]" />
            </div>
            <div class="file-info">
      ...
      <input ref="fileInputRef" type="file" accept=".html,.htm" hidden @change="handleFileSelect" />
    </template>

    <template #introduction>
      <div class="text-sm text-g-600 leading-relaxed">
        <p class="mb-3">HTML转PDF工具可以将HTML文件转换为PDF格式。</p>
...
<script setup lang="ts">
  ...
  import { createHtmlToPdfProcessor } from '@/processors/pdf/convert'

  defineOptions({ name: 'HtmlToPdfPage' })
  ...
  const {
    ...
    handleDrop,
    clearFiles
  } = useUpload({ accept: '.html,.htm', multiple: false, maxSize: 10, maxCount: 1 })

  // 工具处理器
  const processor = createHtmlToPdfProcessor()
  const {
    isProcessing,
    progress,
    hasResult,
    ...
    featureId: 'pdf-from-html',
    featureName: 'HTML转PDF',
    successMessage: '转换完成！',
    errorMessage: '转换失败'
  })
  ...
  const outputFileName = computed(() => {
    ...
    return files.value[0].name.replace(/\.(html|htm)$/i, '') + '.pdf'
  })
