<template>
  <ToolPageLayout
    feature-id="pdf-to-pdfa"
    feature-name="PDF转PDF/A"
    :title="toolName || 'PDF转PDF/A'"
    :description="toolDescription || '将PDF转换为PDF/A归档格式，用于长期保存'"
    :icon="toolIcon"
  ...
  >
    <template #content>
      ...
      >
        ...
        <template v-if="!hasFiles && !isProcessing && !hasResult">
          <Icon icon="ri:archive-fill" class="text-6xl text-[#9b59b6] mb-4" />
        ...
        <template v-if="hasFiles && !isProcessing && !hasResult">
          ...
          <div class="convert-options">
            <ElRadioGroup v-model="options.pdfaVersion">
              <ElRadio value="1b">PDF/A-1b (基础兼容)</ElRadio>
              <ElRadio value="2b">PDF/A-2b (增强功能)</ElRadio>
            </ElRadioGroup>
          </div>
          ...
          >
            <template #default>
              <div class="result-file-card">
                <Icon icon="ri:archive-fill" class="text-5xl text-[#9b59b6]" />
                <p class="result-file-name">{{ outputFileName }}</p>
              </div>
            </template>
            <template #actions>
              <ElButton type="primary" @click="handleDownload">
                <ElIcon class="mr-1"><Download /></ElIcon>下载PDF/A
              </ElButton>
    ...
    <template #introduction>
      <div class="text-sm text-g-600 leading-relaxed">
        <p class="mb-3">PDF/A是专为长期保存设计的PDF归档格式。</p>
        <ul class="list-disc list-inside space-y-1 text-g-500">
          <li>嵌入所有使用的字体</li>
          <li>移除不支持的功能（JavaScript、加密等）</li>
          <li>符合ISO标准，适合档案存储</li>
          <li>PDF/A-1b：基础兼容性</li>
          <li>PDF/A-2b：支持更多功能</li>
        </ul>
      </div>
    </template>
  </ToolPageLayout>
...
<script setup lang="ts">
  ...
  import { createPdfToPdfaProcessor } from '@/processors/pdf/convert'

  defineOptions({ name: 'PdfToPdfaPage' })
  ...
  const {
    ...
    featureId: 'pdf-to-pdfa',
    featureName: 'PDF转PDF/A',
    successMessage: '转换完成！',
    errorMessage: '转换失败'
  })
  ...
  const outputFileName = computed(() => {
    if (!files.value[0]?.name) return 'output_pdfa.pdf'
    return files.value[0].name.replace(/\.pdf$/i, '') + '_pdfa.pdf'
  })
